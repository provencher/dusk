...
Aurora now treats the proof as the hard gate before persistent rendering: after the clear proof succeeds, it creates a persistent OpenXR session, allocates per-eye swapchains, wraps each runtime-owned Vulkan image as a Dawn texture, and lets `aurora_xr_begin_eye()` install the acquired eye image as the active GX/EFB render target.
...
- Promoted the proof-success path into persistent runtime state in Aurora: OpenXR instance/session/reference-space lifecycle, per-eye swapchains, wrapped Dawn textures for each `XrSwapchainImageVulkanKHR::image`, per-eye depth targets, frame wait/begin/end, and projection-layer submission.
- Parameterized Aurora's EFB render-pass targets with `gfx::EfbRenderTargets`. `aurora_xr_begin_eye()` now acquires/waits an OpenXR eye image and installs its wrapped Dawn texture as the active EFB target; `aurora_xr_end_eye()` restores the default EFB target for later flat rendering.
- Rebuilt `dusk_openxr_probe` and `dusk` in `build\windows-msvc-dawn-vendor-openxr`; `ctest --test-dir build\windows-msvc-dawn-vendor-openxr -L openxr --output-on-failure` passes 4/4 tests against the patched Monado simulated HMD runtime.
- Tightened `dusk_openxr_probe_live_clear` so the live mock-runtime test passes only when the proof clears and the harness reports `xr_live_gate=cleared_submitted`, meaning it drove `aurora_begin_frame()`, `aurora_xr_begin_eye()`, `aurora_xr_end_eye()`, and `aurora_end_frame()` through per-eye target submission.
- The live verifier exposed that Monado's `XRT_COMPOSITOR_NULL=TRUE` path is suitable for target lifetime testing but not projection-layer display submission. Aurora now suppresses projection layers only for that null-compositor environment; real runtimes keep the projection-layer submission path enabled.
...
| Only after proof success, parameterize Aurora EFB targets so `aurora_xr_begin_eye()` installs per-eye render targets. | `gfx::EfbRenderTargets` lets XR install wrapped OpenXR swapchain images and matching depth targets as the active GX/EFB render target. The proof still runs first and stops startup if the Dawn image interop gate fails. `dusk_openxr_probe_live_clear` requires `xr_live_gate=cleared_submitted`, so the mock-runtime test exercises the eye target path. | Implemented and locally verified against Monado's simulated HMD. |
...
Current stop condition: the Dawn-first proof gate has cleared on a mock HMD runtime, and Aurora now installs per-eye OpenXR swapchain-backed EFB targets for the existing Dusk stereo loop. The next validation gate is physical headset runtime coverage plus visual/gameplay review of stereo world rendering, projection-layer submission, and the remaining flat UI path.
