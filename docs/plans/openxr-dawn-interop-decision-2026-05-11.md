...
| Only after proof success, parameterize Aurora EFB targets so `aurora_xr_begin_eye()` installs per-eye render targets. | `gfx::EfbRenderTargets` lets XR install wrapped OpenXR swapchain images and matching depth targets as the active GX/EFB render target. The proof still runs first and stops startup if the Dawn image interop gate fails. | Implemented and locally verified against Monado's simulated HMD. |
